...
class SessionTransport
{
    ...
    {
        if (Auth::check()) {
            $guest_view_history = collect(session()->get('user_product_view_history', []));
            if ($guest_view_history->count() > 0) {
                ...
                session()->forget('user_product_view_history');
            }

            $guest_cart = collect(session()->get('guest_cart_store', []));
            if ($guest_cart->count() > 0) {
                ...
                session()->forget('guest_cart_store');
            }
        }

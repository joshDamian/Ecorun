<x-social-layout>
    <div class="sticky z-40 px-3 py-1 bg-gray-100 bg-opacity-75 border-b border-gray-200 top-11 sm:px-5">
        <div class="flex">
            <a class="mr-4" href="{{ $comment->url->show }}">
    ...
    <div class="px-3 py-3 bg-white bg-opacity-75">
        @livewire('connect.post.comment.edit-comment', ['comment' => $comment, 'profile' =>
        Auth::user()->currentProfile, 'confirm' => $confirm_delete ?? null], key("edit_comment" .
        $comment->id))
    </div>
</x-social-layout>

<div>
                        ...
                        <div class="px-4 py-3 {{ $index > 0 ? 'border-t border-gray-200' : '' }}" wire:click="$set('addTeamMemberForm.role', '{{ $role->key }}')">
                            <div class="{{ isset($addTeamMemberForm['role']) && $addTeamMemberForm['role'] !== $role->key ? 'opacity-50' : '' }}">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <svg class="w-5 h-5 ml-2 text-green-400" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mt-10 sm:mt-0">
        <x-jet-action-section>
            ...
            <x-slot name="content">
                <div class="space-y-6">
                    @foreach ($team->users->sortBy('profile.name') as $user)
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <img class="w-8 h-8 rounded-full" src="{{ $user->profile->profile_photo_url }}" alt="{{ $user->profile->name }}">
                            <div class="ml-4">{{ $user->profile->name }}</div>
                        </div>

<x-business-layout>
    ...
    <i class="flex-shrink-0 mx-2 text-gray-600 fas fa-chevron-right"></i>
    <a href="{{ $service->url->show }}" class="flex-shrink-0 cursor-pointer hover:underline">{{ $service->name }}</a>
    @endsection
    <div>
        <x-buy.market-place />
        <div x-data="service_data()" x-init="init_service()" class="md:mt-3">
                    ...
                    <div class="grid grid-cols-1 gap-2 p-3 bg-white sm:p-3 md:mx-2 md:shadow-lg md:rounded-md md:p-3">
                        <div>
                            ...
                            <div class="py-3 md:py-6">
                                @auth
                                <div class="mb-4">
                                    @livewire('connect.sellable.bookmark-sellable', ['sellable' => $service],
                                    key("sellable_bookmark_{$service->id}"))
                                </div>
                                @endauth
                                <div
            ...
            <!-- expansible description  & options -->
            <div class="grid grid-cols-1 gap-0 bg-gray-100 md:mb-3 md:mt-3">
                ...
                <table class="table border-collapse rounded-b-sm">
                    <tr>
                        ...
                        <td class="px-3 py-2 text-lg font-medium text-center border border-gray-200">
                            {{ $value }}</td>
                        @endforeach
                    </tr>
                </table>
    ...
    <script>
        function service_data() {
                return {
                    ...
                    show_specs: null,
                    init_service() {
                        this.activeImage = '{{ $service->gallery->first()->image_url }}';
                    }
                }
            }
    </script>

<div x-data="{ shared: false, timeout: null, event: 'newShare.{{ $feedback_id . '.' . str_replace('\\', '.', get_class($product)) }}' }"
    ...
    <div
        class="@auth grid grid-cols-3 gap-3 @endauth @guest flex justify-center @endguest px-3 py-4 border-t border-gray-200 sm:gap-4 sm:px-5 sm:py-3">
        <div>
        ...
        <a class="flex items-center justify-center px-3 py-2 font-bold text-center text-blue-700 bg-white rounded-full"
            href="{{ $product->url->show }}">
            <i class="text-xl text-blue-700 fas fa-shopping-bag"></i> <span class="hidden sm:inline">&nbsp; Shop</span>
        </a>

        @auth
        <div x-data="{ liked: '{{ $this->liked() }}', clicked: null }" class="px-3 py-2 bg-white rounded-full">
        ...
        <div class="flex items-center justify-center px-3 py-2 bg-white rounded-full">

...
<div x-data="{ activeImage: false, gallery: {{ json_encode($gallery) }}, view: '{{ $view }}', curve: ('{{ $curve }}' !== '') ? true : false, makeActive: (image) => {
    ...
    <div class="grid gap-1"
        ...
        <template x-for="(image, index) in gallery" :key="index">
            <div x-show="index <= 2"
                ...
                x-on:click="makeActive(image.image_url).then(image => { console.log(image) }).catch(x => console.error(x))"
